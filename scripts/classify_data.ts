...
const OLLAMA_MODEL = process.env.OLLAMA_CHAT_MODEL || 'gemma2:9b'; // Using a faster model for classification if available, else standard
const CONCURRENCY = 3; // Limit concurrent requests to avoid OOM
const TEST_LIMIT = 0; // Set to 0 for specific number of files, or 0 for all.
...
async function classifyText(text: string): Promise<string> {
    ...
    // Truncate text if too long to save context window and speed up
...
`;
    ...
    try {
        ...
        // Fallback if parsing fails but model gave something close
